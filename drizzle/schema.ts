...
export const snapshots = mysqlTable("snapshots", {
  ...
  sourceEventIds: json("sourceEventIds").$type<number[]>(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
export const nextActions = mysqlTable("nextActions", {
  ...
  source: mysqlEnum("source", ["manual", "ai_suggested"]).default("manual").notNull(),
...
export const kbDocuments = mysqlTable("kbDocuments", {
  ...
  description: text("description"),
  fileType: mysqlEnum("fileType", ["pdf", "doc", "md", "txt", "xlsx", "docx"]).default("md").notNull(),
  ...
  processingStatus: mysqlEnum("processingStatus", ["pending", "processing", "completed", "failed"]).default("completed").notNull(),
